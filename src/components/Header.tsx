...
import logo from "../assets/best.svg";
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col max-h-[500px] overflow-auto">
          <ul className="p-4">
            {wallets.map((wallet) => (
              <li
                className="flex items-center justify-between font-semibold text-sm text-white mb-3 cursor-pointer hover:bg-white/20 p-4 rounded-xl bg-white/10 backdrop-blur-sm transition-colors"
                key={wallet.id}
                onClick={() => {
                  if (wallet.id == 4) {
                    openSecondModal();
          ...
          <div className="text-sm text-white/70 flex items-center justify-center gap-2 text-center p-5 bg-white/5 backdrop-blur-sm">
            Powered by{" "}
            <svg
              fill="currentColor"
              viewBox="0 0 114 21"
        ...
        <div className="p-4">
          <div className="relative mb-4">
            <input
              ...
              className="w-full p-4 rounded-xl bg-white/10 backdrop-blur-sm text-white placeholder-white/50 border border-white/10 focus:border-[#75ffff] outline-none transition-colors"
            />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-auto max-h-96">
            {filteredWallets.map((wallet) => (
              <div
                ...
                className="flex flex-col cursor-pointer items-center justify-between bg-white/10 hover:bg-white/20 backdrop-blur-sm p-4 rounded-xl transition-colors"
              >
                <img
                  src={wallet.icon}
                  alt={wallet.name}
                ...
                />
                <div className="text-white mt-3 w-full text-center text-sm font-semibold truncate">
                  {wallet.name}
                </div>
              </div>
            ))}
      ...
      <div className="fixed top-0 left-0 right-0 z-10 bg-[#5a63ff]/70 backdrop-blur-sm">
        <div className="container mx-auto pt-4 w-full">
          <div className="flex flex-row justify-between items-center px-2 lg:px-10">
            ...
            <div className="flex flex-row items-center gap-4">
              ...
              <button
                className="bg-[#75ffff] text-xs font-semibold lg:text-base text-[#5861f6] px-4 py-2 rounded-md ml-7"
                onClick={() => setFirstModalOpen(true)}
              >
                Connect Best Wallet
              </button>
              {/* Desktop Language Code */}
              <div className="hidden lg:flex items-center">
                <select
                  value={selectedLang}
                  onChange={(e) => setSelectedLang(e.target.value)}
                  className="bg-transparent text-white font-semibold uppercase appearance-none cursor-pointer"
                >
                  {languages.map((lang) => (
                    <option
                      key={lang.code}
        ...
        <div className="bg-[#fe7916] text-center py-2 mt-4">
          <p className="text-center text-lg loop-scroll lg:animate-none whitespace-nowrap">
            <strong>Best Wallet token sale is live!</strong> • Go to{" "}
            <span className="underline">bestwallettoken.com</span> to buy $BEST
            tokens!
          </p>
        </div>
      </div>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[280px] bg-[#5a63ff] transform transition-transform duration-300 ease-in-out z-20 ${
        ...
        <div className="flex flex-col p-6">
          ...
          <div className="my-4 relative">
            ...
            <select
              value={selectedLang}
              onChange={(e) => setSelectedLang(e.target.value)}
              className="w-full pl-10 pr-4 py-2 rounded-md bg-white text-black font-semibold appearance-none"
            >
              {languages.map((lang) => (
                <option
                  key={lang.code}
          ...
          <div className="flex flex-col gap-6">
            {navLinks.map((link) => (
              <Link
                to={link.href}
                key={link.label}
                className="text-black bg-white px-4 py-2 rounded-md font-semibold text-base"
                onClick={() => setIsMobileMenuOpen(false)}
              >
                {link.label}
              </Link>
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black/50 transition-opacity duration-300 z-15 lg:hidden ${
